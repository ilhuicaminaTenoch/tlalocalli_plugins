...
if (intval($isCreate) > 0) {
	$grid = Essential_Grid_Db::get_essential_grid_by_id(intval($isCreate));
} elseif ($editAlias) {
	$grid = Essential_Grid_Db::get_essential_grid_by_handle($editAlias);
}
if (!empty($grid)) {
		...
		<div id="eg-create-settings" class="esg-settings-container active-esc">
			<div>
				...
				<div class="eg-cs-tbc">
					...
					<div>
						<label for="name" class="eg-tooltip-wrap" title="<?php esc_attr_e('Name of the grid', ESG_TEXTDOMAIN); ?>">
							<?php esc_html_e('Title', ESG_TEXTDOMAIN); ?> *
						</label>
						<input type="text" name="name" value="<?php echo esc_attr($base->getVar($grid, 'name', '', 's')); ?>" />
					</div>
					<div class="div13"></div>
					<div>
						<label for="handle" class="eg-tooltip-wrap" title="<?php esc_attr_e('Technical alias without special chars and white spaces', ESG_TEXTDOMAIN); ?>">
							<?php esc_html_e('Alias', ESG_TEXTDOMAIN); ?> *
						</label>
						<input type="text" name="handle" value="<?php echo esc_attr($base->getVar($grid, 'handle', '', 's')); ?>" />
					</div>
					<div class="div13"></div>
					<div>
						<label for="shortcode" class="eg-tooltip-wrap" title="<?php esc_attr_e('Copy this shortcode to paste it to your pages or posts content', ESG_TEXTDOMAIN); ?>" >
							<?php esc_html_e('Shortcode', ESG_TEXTDOMAIN); ?>
						</label>
					...
					<div class="div13"></div>
					<div>
						<label for="id" class="eg-tooltip-wrap" title="<?php esc_attr_e('Add a unique ID to be able to add CSS to certain Grids', ESG_TEXTDOMAIN); ?>">
							<?php esc_html_e('CSS ID', ESG_TEXTDOMAIN); ?>
						</label>
					...
					<div id="post-pages-wrap">
						<div>
							...
							<div class="eg-cs-tbc">
								...
								--><select name="post_types" size="5" multiple="multiple">
									...
									if(!empty($post_elements)){
										...
										foreach($post_elements as $handle => $name){
											...
											?>
											<option value="<?php echo esc_attr($handle); ?>"<?php echo $selected; ?>><?php echo esc_html($name); ?></option>
											<?php
											$postTypeCount++;
										}
									}
								...
								<div id="eg-post-cat-wrap">
									...
									--><select id="post_category" name="post_category" size="7" multiple="multiple" >
										...
										foreach ($categories as $handle => $cat) {
											...
											?>
											<option value="<?php echo esc_attr($handle); ?>"<?php echo $selected; ?><?php echo $isDisabled ? ' disabled="disabled"' : ''; ?>><?php echo esc_html($cat); ?></option>
											<?php
										}
										?>
									</select>
								...
								--><span><input type="radio" value="AND" name="category-relation" <?php checked($base->getVar($grid, ['postparams', 'category-relation'], 'OR'), 'AND'); ?>><span class="eg-tooltip-wrap" title="<?php esc_attr_e('Post need to be in all categories/tags selected', ESG_TEXTDOMAIN); ?>"><?php esc_html_e('AND', ESG_TEXTDOMAIN); ?></span></span>
...
<script type="text/javascript">
	...
	var eg_jsonTaxWithCats = <?php echo $base->jsonEncodeForClientSide($postTypesWithCats); ?>;
	var pages = [
		<?php
		if(!empty($pages)){
			$first = true;
	...
	function esg_grid_create_ready_function() {
		...
		AdminEssentials.set_basic_columns(<?php echo $base->jsonEncodeForClientSide($base->set_basic_colums([])); ?>);
		AdminEssentials.set_basic_columns_width(<?php echo $base->jsonEncodeForClientSide($base->set_basic_colums_width([])); ?>);
		AdminEssentials.set_basic_masonry_content_height(<?php echo $base->jsonEncodeForClientSide($base->set_basic_masonry_content_height([])); ?>);
		...
		AdminEssentials.Addons.init({
			afterInit: function() {
				...
				AdminEssentials.set_default_nav_skin(<?php echo $base->jsonEncodeForClientSide($navigation_skins); ?>);
				...
				AdminEssentials.setInitCustomJson(<?php echo $base->jsonEncodeForClientSide($base->get_custom_elements_for_javascript()); ?>);
