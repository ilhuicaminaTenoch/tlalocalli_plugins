			...
			if (!is_wp_error($request)) {
				...
				if ($response !== 'invalid') {
					...
					if ($ret !== false) {
						...
						$return = ['error' => esc_attr__('Can\'t write the file into the uploads folder of WordPress, please change permissions and try again!', ESG_TEXTDOMAIN)];
					}
				} else {
					$return = ['error' => esc_attr__('Purchase Code is invalid', ESG_TEXTDOMAIN)];
				}
			}
			//else, check for error and print it to customer
		} else {
			$return = ['error' => esc_attr__('Can\'t write into the uploads folder of WordPress, please change permissions and try again!', ESG_TEXTDOMAIN)];
		}
		return $return;
	}
					...
					if (wp_mkdir_p($upload_dir['basedir'] . $this->library_path)) {
						...
						if ((!file_exists($file) && !file_exists($file_plugin)) || isset($temp['push_image'])) {
							if ($curl !== false) {
								$done = false;
								$count = 0;
								$args = ['user-agent' => 'WordPress/' . $wp_version . '; ' . get_bloginfo('url') . ' - ' . $count];
