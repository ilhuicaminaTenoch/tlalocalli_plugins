	...
	public function import_grids($import_grids, $import_ids, $check_append = true, $imported_data = [], $create_taxonomies = 'off')
	{
		...
		foreach ($import_grids as $i_grid) {
			...
			if ($append) {
				...
				// overwrite
				$response = $wpdb->update($table_name, $data, ['id' => $i_grid['id']]);
				if ($response !== false) {
	...
	public function import_skins($import_skins, $import_ids, $check_append = true, $ignore_exists = false)
	{
		...
		foreach ($import_skins as $i_skin) {
			...
			if ($append) {
				...
				$wpdb->insert($table_name, ['name' => $i_skin['name'], 'handle' => $i_skin['handle'], 'params' => $i_skin['params'], 'layers' => $i_skin['layers']]);
			} else {
				// overwrite
				$wpdb->update($table_name, ['name' => $i_skin['name'], 'handle' => $i_skin['handle'], 'params' => $i_skin['params'], 'layers' => $i_skin['layers']], ['id' => $i_skin['id']]);
			}
	...
	public function import_elements($import_elements, $import_ids, $check_append = true)
	{
		...
		foreach ($import_elements as $i_element) {
			...
			if ($append) {
				...
				$wpdb->insert($table_name, ['name' => $i_element['name'], 'handle' => $i_element['handle'], 'settings' => $i_element['settings']]);
			} else {
				//overwrite
				$wpdb->update($table_name, ['name' => $i_element['name'], 'handle' => $i_element['handle'], 'settings' => $i_element['settings']], ['id' => $i_element['id']]);
			}
		}
	}
	...
	public function import_navigation_skins($import_navigation_skins, $import_ids, $check_append = true, $ignore_exists = false)
	{
		...
		foreach ($import_navigation_skins as $i_nav_skin) {
			...
			if ($append) {
				...
				$wpdb->insert($table_name, ['name' => $i_nav_skin['name'], 'handle' => $i_nav_skin['handle'], 'css' => $i_nav_skin['css']]);
			} else {
				// overwrite
				$wpdb->update($table_name, ['css' => $i_nav_skin['css']], ['id' => $i_nav_skin['id']]);
			}
