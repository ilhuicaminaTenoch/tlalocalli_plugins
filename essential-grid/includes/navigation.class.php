...
class Essential_Grid_Navigation
{
	...
	public static function get_essential_navigation_skins()
	{
		...
		$table_name = Essential_Grid_Db::get_table('nav_skins', true);
	...
	public static function get_essential_navigation_skin_by_id($id = 0)
	{
		...
		$skin = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . Essential_Grid_Db::get_table('nav_skins', true) . " WHERE id = %d", $id), ARRAY_A);
		if (!empty($skin)) {
	...
	public static function get_essential_navigation_skin_by_handle($handle = '')
	{
		...
		$skin = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . Essential_Grid_Db::get_table('nav_skins', true) . " WHERE handle = %s", $handle), ARRAY_A);
		if (!empty($skin)) {
			$skin['css'] = Essential_Grid_Base::stripslashes_deep($skin['css']);
		}
	...
	public static function insert_default_navigation_skins($data)
	{
		...
		if (!empty($data)) {
			foreach ($data as $skin) {
				...
				$wpdb->insert($table_name, ['name' => $skin['name'], 'handle' => $skin['handle'], 'css' => $skin['css']]);
			}
		}
	}
	...
	public static function update_create_navigation_skin_css($data)
	{
		...
		if (empty($data['name']) && empty($data['sid'])) return esc_attr__('No Navigation Skin ID or Name provided', ESG_TEXTDOMAIN);
		if (empty($data['skin_css'])) return esc_attr__('Empty Navigation Skin CSS', ESG_TEXTDOMAIN);
		...
		if (isset($data['name'])) {
			...
			if (strlen($handle) < 2) return esc_attr__('Cant create handle from name. Handle has to be at least 2 characters long.', ESG_TEXTDOMAIN);

			$response = $wpdb->insert($table_name, ['name' => $data['name'], 'handle' => $handle, 'css' => $data['skin_css']]);
			return boolval($response);
		}

		if (isset($data['sid']) && intval($data['sid'])) {
			// update skin
			$response = $wpdb->update($table_name, ['css' => $data['skin_css']], ['id' => $data['sid']]);
			return boolval($response);
		}
	...
	public static function delete_navigation_skin_css($data)
	{
		...
		$response = $wpdb->delete(Essential_Grid_Db::get_table('nav_skins'), ['handle' => $data['skin']]);
	...
	public function output_filter($demo = false)
	{
		...
		// 2.2.5
		$f .= '<div class="esg-mobile-filter-button esg-display-none"><span>' . esc_html__($this->filter_dropdown_text['filter']) . '</span><i class="eg-icon-down-open"></i></div>';
		...
		$f .= '<div class="esg-filterbutton' . $sel . ' esg-allfilter" data-filter="filterall" data-fid="-1"><span>' . esc_html__($all_filter_text) . '</span></div>';
	...
	public function output_filter_unwrapped($demo = false, $type = '')
	{
		...
		if ($listing == 'dropdown') {
			// 2.2.5
			$f .= '<div class="esg-selected-filterbutton esg-mobile-filter-button"><span>' . esc_html__($filtertext) . '</span><i class="eg-icon-down-open"></i></div>';
			$f .= '<div class="esg-dropdown-wrapper">';
		} else {
			// 2.2.5
			$f .= '<div class="esg-mobile-filter-button"><span>' . esc_html__($filtertext) . '</span><i class="eg-icon-down-open"></i></div>';
		}
		...
		array_unshift($filters_html, '<div class="esg-filterbutton' . $sel . ' esg-allfilter" data-filter="filterall" data-fid="-1"><span>' . esc_html__($all_filter_text) . '</span></div>');
	...
	public function output_sorting()
	{
		...
		if (!empty($this->sorting)) {
			...
			$s .= '>';
			$s .= '<div class="esg-sortbutton"><span>' . esc_html__($this->sort_by_text) . '</span><span class="sortby_data">' . esc_html__($this->set_sorting_text($this->sorting_start)) . '</span>';
			$s .= '<select class="esg-sorting-select" data-start="' . esc_attr($this->sorting_start) . '">';
			foreach ($this->sorting as $sort) {
				$s .= '<option value="' . esc_attr($this->set_sorting_value($sort)) . '" ' . selected($this->sorting_start, $this->set_sorting_value($sort), false) . '>' . esc_html($this->set_sorting_text($sort)) . '</option>';
			}
			$s .= '</select>';
	...
	public function check_for_search()
	{
		...
		if ($this->filter_added === false && $this->search_found === true) {
			...
			$s .= '<div class="esg-filterbutton selected esg-allfilter" data-filter="filterall" data-fid="-1" ><span>' . esc_html__($this->filter_all_text['filter']) . '</span></div>';
			$s .= '</article>';
			$s .= '</div>';
		}
