...
class Essential_Grid_Item_Skin
{
	...
	/* 2.2 */
	private $fancybox_three_options = [];
	...
	public static function update_save_item_skin($data)
	{
		...
		if (isset($data['id']) && intval($data['id']) > 0) { 
			...
			$check = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE handle = %s AND id != %s ", [sanitize_title($data['name']), $data['id']]), ARRAY_A);
			...
			$response = $wpdb->insert($table_name, ['name' => $data['name'], 'handle' => sanitize_title($data['name']), 'params' => json_encode($data['params']), 'layers' => json_encode($data['layers'])]);
	...
	public static function star_item_skin_by_id($data)
	{
		...
		$response = $wpdb->update($table_name, ['settings' => json_encode($settings)], ['id' => $data['id']]);
	...
	public function get_video_poster_src($media_type)
	{
		...
		foreach ($this->default_video_poster_order as $order) {
			...
			switch ($order) {
				...
				case 'youtube-image':
					if (!in_array($media_type, ['youtube', 'content-youtube'])) {
						$found = false;
						break;
					}
	...
	public function output_item_skin($demo = false, $choosen_skin = 0)
	{
		...
		if ($demo === false || $demo === 'preview' || $demo === 'custom') {
			if (!empty($this->sorting)) {
				foreach ($this->sorting as $handle => $value) {
					...
					$sortings .= ' data-' . esc_attr($handle) . '="' . esc_attr($sorting_content) . '"';
				}
			}
		}
		...
		$li_class .= ($is_post) ? ' eg-post-id-' . esc_attr($post_id) : ' eg-post-id-' . esc_attr($this->item_counter);
		...
		if (!empty($meta_cover_bg_color)) {
			$meta_cover_style = ' style="background: ' . esc_attr($meta_cover_bg_color) . ';"';
		}
		...
		$cover_blend_mode = $cover_blend_mode === 'normal' ? '' : ' esg-cover-blend-' . esc_attr($cover_blend_mode);
		...
		if ($cover_type == 'full') { 
			...
			if (empty($force_show_cover) || $force_show_cover === 'false') {
				...
				if (preg_match('/line|spiral|circle/', $cover_animation_center)) $cover_animation_color_center = ' data-animcolor="' . esc_attr($base->getVar($this->params, 'cover-animation-color-center', '#FFFFFF')) . '"';
			} else {
				$cover_animation_center = 'esg-none';
			}

			if ($cover_animation_center != 'esg-none' && $cover_animation_center != ' esg-noneout') {
				$cover_animation_delay_center = ' data-delay="' . esc_attr(round($base->getVar($this->params, 'cover-animation-delay-center', 0, 'i') / 100, 2)) . '"';
				$cover_animation_duration_center = ' data-duration="' . esc_attr($base->getVar($this->params, 'cover-animation-duration-center', 'default')) . '"';
			}
			...
			if (empty($force_show_cover) || $force_show_cover === 'false') {
				...
				if (preg_match('/line|spiral|circle/', $cover_animation_top)) $cover_animation_color_top = ' data-animcolor="' . esc_attr($base->getVar($this->params, 'cover-animation-color-top', '#FFFFFF')) . '"';
				if (preg_match('/line|spiral|circle/', $cover_animation_center)) $cover_animation_color_center = ' data-animcolor="' . esc_attr($base->getVar($this->params, 'cover-animation-color-center', '#FFFFFF')) . '"';
				if (preg_match('/line|spiral|circle/', $cover_animation_bottom)) $cover_animation_color_bottom = ' data-animcolor="' . esc_attr($base->getVar($this->params, 'cover-animation-color-bottom', '#FFFFFF')) . '"';
			...
			if ($cover_animation_top != 'esg-none' && $cover_animation_top != ' esg-noneout') {
				$cover_animation_delay_top = ' data-delay="' . esc_attr(round($base->getVar($this->params, 'cover-animation-delay-top', 0, 'i') / 100, 2)) . '"';
				$cover_animation_duration_top = ' data-duration="' . esc_attr($base->getVar($this->params, 'cover-animation-duration-top', 'default')) . '"';
			}
			...
			if ($cover_animation_bottom != 'esg-none' && $cover_animation_bottom != ' esg-noneout') {
				$cover_animation_delay_bottom = ' data-delay="' . esc_attr(round($base->getVar($this->params, 'cover-animation-delay-bottom', 0, 'i') / 100, 2)) . '"';
				$cover_animation_duration_bottom = ' data-duration="' . esc_attr($base->getVar($this->params, 'cover-animation-duration-bottom', 'default')) . '"';
			}
		...
		if ($cover_animation_top) {
			...
			$data_transition_top = ' data-transition="' . esc_attr($cover_animation_top) . '"';
		...
		if ($cover_animation_center) {
			...
			$data_transition_center = ' data-transition="' . esc_attr($cover_animation_center) . '"';
		...
		if ($cover_animation_bottom) {
			...
			$data_transition_bottom = ' data-transition="' . esc_attr($cover_animation_bottom) . '"';
		...
		if ($cover_group_animation != 'esg-none') {
			$cover_group_animation_delay = ' data-delay="' . esc_attr(round($base->getVar($this->params, 'cover-group-animation-delay', 0, 'i') / 100, 2)) . '"';
			$cover_group_animation_duration = ' data-duration="' . esc_attr($base->getVar($this->params, 'cover-group-animation-duration', 'default')) . '"';
		} else {
		...
		if ($cover_group_animation) {
			$data_transition_group = ' data-transition="' . esc_attr($cover_group_animation) . '"';
		...
		$media_blur = strpos($media_animation, 'blur') === false ? '' : ' data-bluramount="' . esc_attr($base->getVar($this->params, 'media-animation-blur', '5')) . '"';

		if ($media_animation != 'esg-none') {
			$media_animation_delay = ' data-delay="' . esc_attr(round($base->getVar($this->params, 'media-animation-delay', 0, 'i') / 100, 2)) . '"';
			$media_animation_duration = ' data-duration="' . esc_attr($base->getVar($this->params, 'media-animation-duration', 'default')) . '"';
		} else {
			$media_animation = '';
		}
		...
		if (!empty($splitItem) && $splitItem !== 'none') {
			$li_class .= ' esg-split-content esg-split-' . esc_attr($splitItem);
		}
		...
		if ($this->grid_type == 'cobbles') {
			...
			$cobbles_data = ' data-cobblesw="' . esc_attr($use_cobbles[0]) . '" data-cobblesh="' . esc_attr($use_cobbles[1]) . '"';
		}
		...
		$item_animation = '';
		if ($itm_anime !== 'none') {
			$item_animation .= ' data-anime="esg-item-' . esc_attr($itm_anime) . '"';
			switch ($itm_anime) {
				case 'zoomin':
					$item_animation .= ' data-anime-zoomin="' . esc_attr($this->grid_item_animation_zoomin) . '"';
					break;
				case 'zoomout':
					$item_animation .= ' data-anime-zoomout="' . esc_attr($this->grid_item_animation_zoomout) . '"';
					break;
				case 'fade':
					$item_animation .= ' data-anime-fade="' . esc_attr($this->grid_item_animation_fade) . '"';
					break;
				case 'blur':
					$item_animation .= ' data-anime-blur="' . esc_attr($this->grid_item_animation_blur) . '"';
					break;
				case 'shift':
					$item_animation .= ' data-anime-shift="' . esc_attr($this->grid_item_animation_shift) . '"';
					$item_animation .= ' data-anime-shift-amount="' . esc_attr($this->grid_item_animation_shift_amount) . '"';
					break;
				case 'rotate':
					$item_animation .= ' data-anime-rotate="' . esc_attr($this->grid_item_animation_rotate) . '"';
					break;
			}
		}
		...
		if ($itm_anime_other !== 'none') {
			$item_animation .= ' data-anime-other="esg-item-' . esc_attr($itm_anime_other) . '"';
			switch ($itm_anime_other) {
				case 'zoomin':
					$item_animation .= ' data-anime-other-zoomin="' . esc_attr($this->grid_item_other_zoomin) . '"';
					break;
				case 'zoomout':
					$item_animation .= ' data-anime-other-zoomout="' . esc_attr($this->grid_item_other_zoomout) . '"';
					break;
				case 'fade':
					$item_animation .= ' data-anime-other-fade="' . esc_attr($this->grid_item_other_fade) . '"';
					break;
				case 'blur':
					$item_animation .= ' data-anime-other-blur="' . esc_attr($this->grid_item_other_blur) . '"';
					break;
				case 'shift':
					$item_animation .= ' data-anime-other-shift="' . esc_attr($this->grid_item_other_shift) . '"';
					$item_animation .= ' data-anime-other-shift-amount="' . esc_attr($this->grid_item_other_shift_amount) . '"';
					break;
				case 'rotate':
					$item_animation .= ' data-anime-other-rotate="' . esc_attr($this->grid_item_other_rotate) . '"';
					break;
			}
		}
		...
		echo '<li ' . $cltitle . 'id="' . esc_attr($li_id) . '"' . $li_skin . ' class="filterall' . $filters . $li_class;
		...
		if ($demo == 'overview' || $demo == 'skinchoose') {
			...
			$showid = (isset($_GET['showid'])) ? ' (ID: ' . esc_html($this->id) . ')' : '';
			...
			echo '<div class="eg-item-skin-overview-name">' . esc_html($this->name) . $showid . "</div>\n";

			if ($demo == 'overview') {
				$fav_class = (!isset($this->settings['favorite']) || !$this->settings['favorite']) ? 'eg-icon-star-empty' : 'eg-icon-star';

				echo '<a href="javascript:void(0);" title="' . esc_attr__('Mark as Favorite', ESG_TEXTDOMAIN) . '" class="eg-ov-1 eg-overview-button eg-btn-star-item-skin esg-purple eg-tooltip-wrap" id="eg-star-' . esc_attr($this->id) . '"><i class="' . $fav_class . '"></i></a>';
				...
				echo '<a href="javascript:void(0);" title="' . esc_attr__('Duplicate Skin', ESG_TEXTDOMAIN) . '" class="eg-ov-3 eg-overview-button eg-btn-duplicate-item-skin esg-blue eg-tooltip-wrap " id="eg-duplicate-' . esc_attr($this->id) . '"><i class="eg-icon-picture"></i></a>';
				echo '<a href="javascript:void(0);" title="' . esc_attr__('Delete Skin', ESG_TEXTDOMAIN) . '" class="eg-ov-4 eg-overview-button eg-btn-delete-item-skin esg-red eg-tooltip-wrap " id="eg-delete-' . esc_attr($this->id) . '"><i class="eg-icon-trash"></i></a>';
				...
				echo '<input class="eg-tooltip-wrap eg-tooltip-wrap-position " type="radio" value="' . esc_attr($this->id) . '" title="' . esc_attr__('Choose Skin', ESG_TEXTDOMAIN) . '" name="entry-skin"';
				if ($choosen_skin == '-1')
			...
			echo '<a href="javascript:void(0);" class="eg-ov-2 eg-overview-button eg-btn-activate-post-item ' . $vis_icon_color . ' eg-tooltip-wrap" title="' . esc_attr__('Show/Hide from Grid', ESG_TEXTDOMAIN) . '" id="eg-act-post-item-' . esc_attr($this->post['ID']) . '"><i class="' . $vis_icon . '"></i></a>';
			...
			echo '<a href="javascript:void(0);" class="eg-ov-4 eg-overview-button eg-btn-edit-post-item esg-green eg-tooltip-wrap" title="' . esc_attr__('Edit Post Meta', ESG_TEXTDOMAIN) . '" id="eg-edit-post-item-' . esc_attr($this->post['ID']) . '"><i class="eg-icon-cog"></i></a>';
			echo '</div>' . "\n";
			echo '          </div>' . "\n";
			echo '          <div class="clear"></div>' . "\n\n";
			echo '       </div>' . "\n\n";
		...
		if ($demo === false || $demo == 'preview' || $demo == 'custom') {
			...
			if (!empty($hover_image) && $hover_image !== 'false' && !empty($this->media_sources) && isset($this->media_sources['alternate-image']) && !empty($this->media_sources['alternate-image'])) {
				...
				$echo_media .= '<div class="esg-hover-image' . $hover_image_animation . '" data-src="' . esc_attr($hover_image) . '"' . $hover_image_animation_delay . $data_transition_hover . '></div>';
			}
		...
		// 2.2.5
		if ($media_animation) {
			$data_transition_media = ' data-transition="' . esc_attr($media_animation) . '"';
		...
		if ($demo == 'overview' || $demo == 'skinchoose') {
			echo '            <div class="esg-entry-media' . $media_animation . '"' . $media_animation_delay . $media_animation_duration . $media_blur . $data_transition_media . '><img class="esg-entry-media-img" src="' . ESG_PLUGIN_URL . 'admin/assets/images/' . esc_attr($this->cover_image) . '" data-no-lazy="1"></div>' . "\n\n";
		} else {
			...
			if ($this->load_lightbox === true) {
				if (!empty($this->lightbox_additions['items'])) {
					...
					foreach ($this->lightbox_additions['items'] as $lb_key => $lb_img) {
						...
						echo '<a href="' . esc_attr($lb_img) . '" data-thumb="' . str_replace([" ", "(", ")"], ["%20", "%28", "%29"], $thumg_src) . '" class="esgbox" ' . $lb_rel . '><img class="esg-lb-dummy" src="' . $thumg_src . '"></a>';
					}
					echo '</div>';
				}
			}
				...
				if (empty($postCatsIDs) && isset($this->post['post_type'])) {
					...
					if (!empty($obj) && is_array($obj)) {
						foreach ($obj as $tax) {
							...
							if($use_taxonomies === false) $use_taxonomies = [];
							$use_taxonomies[] = $tax;
